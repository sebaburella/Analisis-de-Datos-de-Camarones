...
@app.callback(
    [Output('output-container-date-picker-single', 'children'),
     ...
     Output('prediccion-costos-combinado', 'figure')],  # Agregar el Output para el gráfico de costos
    [Input('my-date-picker-single', 'date'),
     Input('model-type', 'value')])
def grafico_peso(selected_date, model_type):
    ...
    return 'No date selected', go.Figure(), '', px.pie(), px.bar(), go.Figure()  # Gráficos vacíos si no hay fecha seleccionada
