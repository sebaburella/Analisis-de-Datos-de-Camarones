...
data_table = dash_table.DataTable(
    ...
    columns=[
        ...
        {"name": "IPS", "id": "IPS", "type": "numeric", "format": Format(precision=2, scheme=Scheme.fixed)},
        ...
        {"name": "Kg/Has", "id": "Kg/Has", "type": "numeric", "format": Format(precision=2, scheme=Scheme.fixed)}
    ],
...
app.layout = html.Div([
    html.Div([
        dcc.DatePickerSingle(
            ...
            min_date_allowed=df['Fecha'].min().strftime('%d-%m-%Y'),  # Cambiar el formato de la fecha mínima permitida
            max_date_allowed=future_df['Fecha'].max().strftime('%d-%m-%Y'),  # Cambiar el formato de la fecha máxima permitida
            initial_visible_month=df['Fecha'].min(),
            date=future_df['Fecha'].max()  # Set the initial selected date
        ),
    ...
    html.Div([
        ...
        dcc.Graph(id="graph"),
        html.Div(id='prediction-table')  # Aquí se mostrará la tabla de predicciones
...
def update_output(selected_date, model_type):
    ...
    if selected_date:
        ...
        return string_prefix + selected_date, fig, prediction_table

    return 'No date selected', go.Figure(), ''
