...
app.layout = html.Div([
    html.Div([
        dcc.DatePickerRange(
            ...
            display_format='DD-MM-YYYY'  # Cambiar el formato a "dia-mes-año"
        ),
        dcc.Dropdown(
            id='model-type',
...
def update_output(start_date, end_date, model_type):
    ...
    if start_date and end_date:
        ...
        filtered_df = extended_df[(extended_df['Fecha'] >= start_date) & (extended_df['Fecha'] <= end_date)]

        # Agregar la traza para los datos reales
        fig.add_trace(go.Scatter(
            x=filtered_df['Fecha'][:len(df)],
            y=filtered_df['Peso_Promedio'][:len(df)],
            mode='lines',
            name='Datos reales',
            line=dict(color='blue')  # Establecer el color de la línea
        ))

        # Obtener el índice del último valor real
        last_real_index = len(df) - 1

        # Asegurarse de que la primera predicción de regresión lineal sea igual al último valor real
        filtered_df.loc[last_real_index, 'Peso_Promedio'] = filtered_df.loc[last_real_index - 1, 'Peso_Promedio']

        # Agregar la traza para las predicciones según el modelo seleccionado
        if model_type == 'multiple':
            fig.add_trace(go.Scatter(
                ...
                line=dict(color='red')  # Establecer el color de la línea
            ))
        elif model_type == 'simple':
            # Utilizar las predicciones del modelo de regresión lineal del DataFrame filtrado
            fig.add_trace(go.Scatter(
                x=filtered_df['Fecha'][len(df):],
                y=filtered_df['Predicciones_Simple'][len(df):],
                mode='lines',
                name='Predicciones Simples',
                line=dict(color='green')  # Establecer el color de la línea
            ))
        elif model_type == 'sarima':
            fig.add_trace(go.Scatter(
                x=filtered_df['Fecha'][len(df)-1:],
                y=filtered_df['Predicciones_SARIMA'][len(df)-1:],
                mode='lines',
                name='Predicciones SARIMA',
                line=dict(color='orange')  # Establecer el color de la línea
            ))
