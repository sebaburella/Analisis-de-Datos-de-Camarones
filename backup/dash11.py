...
@app.callback(
    [
        ...
        Output('prediction-table', 'children'),  # Agregar la nueva salida para la tabla de predicciones
    ],
    [
...
def update_components(selected_date, model_type, variable_objetivo):
    ...
    prediction_table = dash_table.DataTable(
        ...
        data=[{
            ...
            'Predicciones_SARIMA': pred_sarima
        } for formatted_date, peso_promedio, pred_simple, pred_sarima in zip(formatted_dates, date_range_df['Peso_Promedio'][len(df):], date_range_df['Predicciones_Simple'][len(df):], date_range_df['Predicciones_SARIMA'][len(df):])],
        style_table={'height': '100px', 'overflowY': 'auto'}
    )
    ...
    return string_prefix + selected_date, fig, pie_chart, bar_chart,prediction_table
...
def grafico_costo(selected_date):
    # Filtrar el DataFrame df_costo_predicciones por selected_date
    ...
    fig.add_trace(go.Scatter(x=date_range_df_costo['Fecha'], y=date_range_df_costo['Costo_Directo'],
                             mode='lines', name='Datos reales'))
    
    # Agregar la línea de las predicciones
    fig.add_trace(go.Scatter(x=date_range_df_costo['Fecha'], y=date_range_df_costo['Costo_Directo_Predicho'],
