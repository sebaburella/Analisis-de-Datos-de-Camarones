...
def update_components(selected_date, model_type, variable_objetivo):
    ...
    pie_chart = px.pie()
    bar_chart = px.bar()
    prediction_table = dash_table.DataTable()  # Inicializar la variable prediction_table
    ...
    if variable_objetivo == 'Peso_Promedio':
        ...
        prediction_table = update_cost_prediction_table(selected_date)  # Actualizar la tabla de predicciones de costo
...
def update_cost_prediction_table(selected_date):
    date_range_df = extended_df[extended_df['Fecha'] <= selected_date]
    prediction_table = dash_table.DataTable(
        data=date_range_df.round(2).to_dict('records'),
        columns=[{'name': col, 'id': col} for col in date_range_df.columns],
        style_table={'height': '200px', 'overflowY': 'auto'},
    )
