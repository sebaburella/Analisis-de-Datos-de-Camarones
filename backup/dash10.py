...
def update_components(selected_date, model_type, variable_objetivo):
    ...
    if variable_objetivo == 'Peso_Promedio':
        ...
        fig = grafico_costo()  # Actualizar el gráfico de predicciones de Costo Directo
...
def update_components(selected_date, model_type, variable_objetivo):
    string_prefix = 'You have selected: '
    if not selected_date:
        return string_prefix + 'No date selected', go.Figure(), '', px.pie(), px.bar()
    ...
    if variable_objetivo == 'Peso_Promedio':
        ...
        return string_prefix + selected_date, fig, pie_chart, bar_chart

    return string_prefix + selected_date, fig, px.pie(), px.bar()
