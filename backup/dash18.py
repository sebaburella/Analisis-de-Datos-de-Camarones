...
app.layout = html.Div([
    html.Div([
        ...
        dcc.Graph(id='ganancias-aproximadas'),  # Agregamos este gráfico al inicio del layout

        dbc.Row([
            dbc.Col(html.Div(dbc.Alert("QUANTITY ", color='light')), width=2),
            dbc.Col(html.Div(id="r2-score-display"), width=2),
            dbc.Col(html.Div(dbc.Alert("REVENUE ", color='light')), width=2),
            dbc.Col(html.Div(dbc.Alert("REVENUE ", color='light')), width=2),
            dbc.Col(html.Div(dbc.Alert("REVENUE ", color='light')), width=2),
            dbc.Col(html.Div(dbc.Alert("REVENUE ", color='light')), width=2),
            dbc.Col(html.Div(dbc.Alert("REVENUE ", color='light')), width=2),
...
def grafico_predicciones(selected_date, model_type, variable_objetivo):
    ...
    if variable_objetivo == 'Peso_Promedio':
        ...
        pie_chart = px.pie(values=df_unique['Comentario'].value_counts().values,
                        names=df_unique['Comentario'].value_counts().index,hole=.5,
                        title='Distribución de Comentarios')
        pie_chart.update_traces(opacity=0.6)
    ...
    return string_prefix + selected_date, fig, pie_chart, bar_chart, 'Precision :'+r2_display
...
def grafico_costo(selected_date):
    ...
    fig.add_trace(go.Scatter(x=date_range_df_costo['Fecha'], y=date_range_df_costo['Costo_Directo_Acumulado'],
                             mode='lines', name='Datos reales', fill='tozeroy',  # Esto rellena el área desde el valor y hasta el eje x
                                fillcolor='rgba(0, 0, 255, 0.2)',  # Color de relleno con opacidad del 20%
                                showlegend=False))
    fig.add_trace(go.Scatter(x=date_range_df_costo['Fecha'], y=date_range_df_costo['Costo_Directo_Acumulado_Predicho'],
                             mode='lines', name='Predicciones', fill='tozeroy',  # Esto rellena el área desde el valor y hasta el eje x
                                fillcolor='rgba(255, 0, 0, 0.2)',  # Color de relleno con opacidad del 20%
                                showlegend=False,line=dict(dash='dash')))
    fig.update_layout(title='Costo Directo Acumulado - Predicciones',
                      xaxis_title='Fecha',
                      yaxis_title='Costo Directo Acumulado',
                      )
    fig.update_layout(paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)')  # Remove background
    return fig
