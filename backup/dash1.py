...
feature_cols = ['Incremento', 'CFR', 'IPS', 'Sobrevivencia', 'Biomasa', 'Biomasa_Hectarea', 'Kg/Has']
...
# Antes de agregar predicciones al DataFrame, guardamos el último valor real y la última fecha
...
# Calcular el promedio de Peso_Promedio, CFR e IPS
average_weight = df['Peso_Promedio'].mean()
average_cfr = df['CFR'].mean()
...
# Crear el KPI para CFR
kpi_cfr = dbc.Card([
    dbc.CardBody([
        html.H4("Promedio de CFR", className="card-title"),
        html.P(f"{average_cfr:.2f}", className="card-text"),
    ])
], style={
    "width": "18rem", 
    "height": "6rem",
...
data_table = dash_table.DataTable(
    id='table',
    columns=[{"name": i, "id": i} for i in df_weekly.columns if i != "Unnamed: 0"],  # excluir la columna "Unnamed: 0"
    data=df_weekly.to_dict('records'),
